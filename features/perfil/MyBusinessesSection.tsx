...
const STATUS_META: Record<ContentStatus, { label: string; tone: string; icon: typeof Clock3 }> = {
  pending_approval: { label: 'Aguardando aprovação', tone: 'bg-amber-50 text-amber-700 border-amber-200',   icon: Clock3 },
  published:        { label: 'Publicado',            tone: 'bg-green-50 text-green-700 border-green-200',   icon: CheckCircle2 },
  archived:         { label: 'Não aprovado',         tone: 'bg-rose-50 text-rose-700 border-rose-200',      icon: AlertCircle },
  draft:            { label: 'Rascunho',             tone: 'bg-blue-50 text-blue-700 border-blue-200',      icon: Clock3 },
};
...
const emptyForm: FormState = {
  title: '', description: '', category: 'restaurante',
...
export default function MyBusinessesSection() {
  ...
  const startEdit = (biz: Business) => setEditing({
    id: biz.id,
    title: biz.title,
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editing.id) {
        await updateOwnedBusiness(editing.id, {
  ...
  return (
    <section className="glass-panel p-5 md:p-6">
      <div className="mb-5 flex flex-col gap-3 border-b border-border pb-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs font-medium text-text-muted">
            Cadastre seu comércio ou serviço. Após aprovação da prefeitura, ele aparece em <code className="font-mono text-primary">/comercio</code>.
          </p>
        </div>
        {!editing && (
          <button
      ...
      {editing && (
        <form onSubmit={handleSubmit} className="mb-6 grid grid-cols-1 gap-4 rounded-2xl border border-primary/20 bg-white p-4 md:grid-cols-2 md:p-5">
          <div className="md:col-span-2 flex items-center justify-between border-b border-border pb-3">
            ...
            <button type="button" onClick={cancelEdit} aria-label="Cancelar"
          ...
          <label className="md:col-span-2 space-y-1.5">
            ...
            <input type="text" value={editing.title} required maxLength={120}
          ...
          <label className="md:col-span-2 space-y-1.5">
            ...
            <textarea value={editing.description} required rows={3} maxLength={400}
              onChange={(e) => setEditing({ ...editing, description: e.target.value })}
              placeholder="O que oferece, especialidades, diferenciais."
          ...
          <label className="space-y-1.5">
            ...
            <select value={editing.category}
              onChange={(e) => setEditing({ ...editing, category: e.target.value as Business['category'] })}
              className="h-11 w-full rounded-xl border border-border bg-white px-3 text-sm font-bold outline-none focus:border-primary">
              {CATEGORIES.map((item) => (
          ...
          <label className="space-y-1.5">
            ...
            <input type="text" value={editing.hours} maxLength={80}
          ...
          <label className="md:col-span-2 space-y-1.5">
            ...
            <input type="text" value={editing.address} required maxLength={200}
          ...
          <label className="space-y-1.5">
            ...
            <input type="tel" value={editing.phone} maxLength={20}
          ...
          <label className="space-y-1.5">
            <span className="text-xs font-black uppercase tracking-widest text-text-muted">WhatsApp (DDD + número)</span>
            <input type="tel" value={editing.whatsapp} maxLength={13}
              onChange={(e) => setEditing({ ...editing, whatsapp: e.target.value.replace(/\D/g, '') })}
          ...
          <div className="md:col-span-2 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            ...
            <button type="submit" disabled={saving}
              ...
              {editing.id ? 'Salvar alterações' : 'Enviar pra aprovação'}
            </button>
          </div>
        </form>
      )}
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          ...
          {businesses.map((biz) => {
            ...
            return (
              <article key={biz.id} className="civic-card p-4">
                ...
                {biz.status === 'archived' && biz.reviewNote && (
