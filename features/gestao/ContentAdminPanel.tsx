...
const GROUPS: { value: ContentGroup; label: string; description: string }[] = [
  { value: 'all', label: 'Todos', description: 'Todas as áreas' },
  { value: 'publishing', label: 'Publicação', description: 'Informações públicas' },
...
const TABS: { value: ContentTab; label: string; icon: LucideIcon; group: Exclude<ContentGroup, 'all'> }[] = [
  ...
  { value: 'traffic', label: 'Trânsito', icon: Car, group: 'publishing' },
  ...
  { value: 'enrollments', label: 'Matrículas', icon: GraduationCap, group: 'operations' },
  { value: 'emergency', label: 'Emergências', icon: Siren, group: 'operations' },
  { value: 'health', label: 'Unidades', icon: Hospital, group: 'registries' },
  { value: 'pharmacy', label: 'Farmácia', icon: Pill, group: 'registries' },
  { value: 'jobs', label: 'Vagas', icon: Briefcase, group: 'registries' },
  { value: 'education', label: 'Educação', icon: GraduationCap, group: 'registries' },
  { value: 'safety', label: 'Segurança', icon: Shield, group: 'registries' },
...
const CATALOGS: Record<Exclude<ContentTab, 'notices' | 'events' | 'works' | 'businesses' | 'traffic' | 'health' | 'appointments' | 'jobs' | 'applications' | 'enrollments' | 'emergency'>, CatalogAdminConfig> = {
  community: {
    ...
    emptyTitle: 'Nenhum grupo comunitário publicado',
  ...
  safety: {
    ...
    eyebrow: 'Segurança pública',
    title: 'pontos de segurança',
    publicPath: '/seguranca',
    emptyTitle: 'Nenhum ponto de segurança publicado',
    categoryField: 'type',
    fields: [
      {
        name: 'type',
  ...
  polls: {
    ...
    fields: [
      ...
      { name: 'startDate', label: 'Início', type: 'text', placeholder: '2026-06-01' },
  ...
  pharmacy: {
    ...
    fields: [
      ...
      { name: 'location', label: 'Local de retirada', type: 'text', defaultValue: 'Farmácia Municipal', required: true },
