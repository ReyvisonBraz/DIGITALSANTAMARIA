...
import { AlertTriangle, Archive, Car, Loader2, MapPin, Pencil, Save, X } from 'lucide-react';
...
export default function TrafficAdmin() {
  ...
  const startEdit = (alert: TrafficAlert) => {
    ...
    setValidUntil(alert.validUntil);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast('Erro ao salvar o alerta.', 'error');
    } finally {
      setSaving(false);
    }
  };
      ...
      <section className="glass-panel p-5 md:p-6">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <label className="space-y-1.5">
            ...
            <select
              value={type}
              onChange={(e) => setType(e.target.value as TrafficAlert['type'])}
              className="h-11 w-full rounded-xl border border-border bg-white px-3 text-sm font-bold outline-none focus:border-primary"
            >
              {TYPES.map((item) => (
          ...
          <label className="space-y-1.5">
            ...
            <select
              value={severity}
              onChange={(e) => setSeverity(e.target.value as TrafficAlert['severity'])}
              className="h-11 w-full rounded-xl border border-border bg-white px-3 text-sm font-bold outline-none focus:border-primary"
            >
              {SEVERITIES.map((item) => (
                <option key={item.value} value={item.value}>{item.label}</option>
          ...
          <label className="md:col-span-2 space-y-1.5">
            <span className="text-xs font-black uppercase tracking-widest text-text-muted">Vigencia ate</span>
            <input
              type="date"
              value={validUntil}
