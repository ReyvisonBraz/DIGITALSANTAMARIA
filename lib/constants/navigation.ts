...
import { isRouteSuspended } from './feature-status';
import {
  Briefcase,
  Calendar,
  DollarSign,
...
const ALL_NAV_LINKS: readonly NavLink[] = [
  ...
  { label: 'Segurança', href: '/seguranca', icon: Shield, category: 'Serviços' },
  { label: 'Meio Ambiente', href: '/meio-ambiente', icon: Leaf, category: 'Serviços' },
  { label: 'Social', href: '/social', icon: Heart, category: 'Serviços' },
  { label: 'Obras', href: '/obras', icon: HardHat, category: 'Gestão' },
...
/** Links de navegação ativos no lançamento (rotas suspensas removidas). */
export const NAV_LINKS: readonly NavLink[] = ALL_NAV_LINKS.filter(
  (link) => !isRouteSuspended(link.href),
);
...
/** Módulos do dashboard ativos no lançamento (rotas suspensas removidas). */
export const DASHBOARD_MODULES: readonly DashboardModule[] = ALL_DASHBOARD_MODULES.filter(
  (mod) => !isRouteSuspended(mod.href),
);

const ALL_FOOTER_LINKS: readonly { label: string; href: string }[] = [
...
export const FOOTER_LINKS: readonly { label: string; href: string }[] = ALL_FOOTER_LINKS.filter(
  (link) => !isRouteSuspended(link.href),
);
